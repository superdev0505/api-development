...
// return generated tns
    ...
    protected function setServiceId(array $config) {
        $config['service'] = empty($config['service_name']) ? $service_param = 'SID = ' . $config['database'] : $service_param = 'SERVICE_NAME = ' . $config['service_name'];
    ...
    protected function setTNS(array $config) {
        $config['tns'] = "(DESCRIPTION = (ADDRESS = (PROTOCOL = "
                . "{$config['protocol']})(HOST = "
                . "{$config['host']})(PORT = "
                . "{$config['port']})) (CONNECT_DATA =(SERVER = DEDICATED)("
                . "{$config['service']})))";
    ...
    public function createConnection($tns, array $config, array $options) {
        ...
        $conf = env('USE_OCI8', null);
        $connection = null;
        if($conf) {
            $connection = new Oci8($tns, $config['username'], $config['password'], $options);
        }
        else {
            $connection = new PDO("oci:dbname=$tns;charset=UTF8", $config['username'], $config['password'], $options);
        }
